"""This is equivalent to the fixed_test_script.py file, but for the highres
simulations.
"""
...
def get_args(i):
    if os.path.isfile("./args/args_HRbox%d.p"%i):
        args = pickle.load(open("./args/args_HRbox%d.p"%i, 'rb'))
        print "Using saved args for HRbox%d"%i
        return args
    else:
        raise Exception("Must have HR args premade.")
    return

def run_bf(args, doprint=False):
    defaults = args['defaults']
    guess = np.array([ 1.06630163,-0.80478931,-0.06387556, 0.61330476])#defaults[args['kept']]
    ...
    for i in range(len(nparams)):
        ...
        for combo in combos:
            ...
            for box in [11]:#range(lo, hi):
                ...
                bfpath = "bfs/bf_%s_HRbox%d.txt"%(args['name'], box)
                args['bfpath'] = bfpath
                mcmcpath = "chains/chain_%s_HRbox%d"%(args['name'], box)
                likespath = "chains/likes_%s_HRbox%d"%(args['name'], box)
                ...
                plot_bf(box, args, bfpath, "figs/bf_%s_HRbox%d.png"%(args['name'],box))
